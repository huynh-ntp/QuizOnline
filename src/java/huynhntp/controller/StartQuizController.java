...
public class StartQuizController extends HttpServlet {
    ...
    private final static String SUCCESS = "startQuiz.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<QuestionDTO> listQuestion = dao.getTestDetails(testsID);
            HttpSession session = request.getSession();
            if(listQuestion!=null){
                int time = dao.getTimeDoQuiz(testsID);
                session.setAttribute("TIME_TO_DO", time);
                session.setAttribute("LIST_QUESTION", listQuestion);
                session.setAttribute("TESTSID", testsID);
            }
            url = SUCCESS;
            
        } catch (SQLException e) {
            Logger.getLogger(StartQuizController.class).error(e.toString());
        }finally{
